...
export default function Claviculario(){
 ...
 async function save(){if(!selected||!mission.trim()){setMessage("Informe a missão.");return}setSaving(true);const supabase=createClient();const{data:{user}}=await supabase.auth.getUser();if(!user){setMessage("Sessão expirada.");setSaving(false);return}
  ...
  const{error}=await supabase.from("key_events").insert({key_slot_id:selected.id,event_type:type,holder_user_id:user.id,holder_name:p?.full_name||user.email||"Usuário",holder_rank:p?.rank,holder_military_id:p?.military_id,mission,destination,notes:external?"Viatura externa: "+external:null,signature_path:path,signature_sha256:hash,witnessed_by:user.id,client_event_id:clientId});
  if(error){setMessage(error.message);setSaving(false);return}await supabase.from("key_slots").update({status:type==="checkout"?"checked_out":"available"}).eq("id",selected.id);setMessage(type==="checkout"?"Retirada assinada.":"Devolução assinada.");setSelected(null);setMission("");clear();await load();setSaving(false)}
 ...
 {selected&&<div className="key-modal-bg"><section className="key-modal"><div className="key-modal-title"><strong>{selected.slot_number}</strong><div><small>{out(selected.id)?"DEVOLUÇÃO":"RETIRADA"}</small><h2>Posição {selected.slot_number}</h2></div></div><label>Viatura externa/temporária<input value={external} onChange={e=>setExternal(e.target.value)} placeholder="Ex.: Prefeitura — 10DP174"/></label><label>Missão<input value={mission} onChange={e=>setMission(e.target.value)} required/></label><label>Destino<input value={destination} onChange={e=>setDestination(e.target.value)}/></label><label className="signature-label"><span><PenLine/>Assine com o dedo</span><canvas ref={canvas} width="700" height="220" onPointerDown={start} onPointerMove={draw} onPointerUp={()=>drawing.current=false}/><button onClick={clear}><RotateCcw/>Limpar</button></label>{message&&<p>{message}</p>}<div className="key-modal-actions"><button onClick={()=>setSelected(null)}>Cancelar</button><button className="key-save" onClick={save} disabled={saving}><Save/>{saving?"Salvando...":"Assinar e salvar"}</button></div></section></div>}</main>
}

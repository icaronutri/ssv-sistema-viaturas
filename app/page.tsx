...
type Counts={checklists:number;reports:number};
...
export default function Home(){
 ...
 const[vehicles,setVehicles]=useState<Vehicle[]>([]),[movements,setMovements]=useState<Movement[]>([]),[maintenance,setMaintenance]=useState<Maintenance[]>([]),[counts,setCounts]=useState<Counts>({checklists:0,reports:0});
 ...
 useEffect(()=>{
  ...
  const apply=async(next:Session|null)=>{if(!active)return;setSession(next);if(!next){setProfile(null);setReady(true);return}
   ...
   const[v,m,mt,c,r]=await Promise.all([
   ...
   if(!active)return;
   setVehicles((v.data??[]) as unknown as Vehicle[]);setMovements((m.data??[]) as unknown as Movement[]);setMaintenance((mt.data??[]) as unknown as Maintenance[]);setCounts({checklists:c.count??0,reports:r.count??0});
   const first=[v.error,m.error,mt.error,c.error,r.error].find(Boolean);if(first)setMessage(`Parte dos dados não pôde ser carregada: ${first.message}`);
   setReady(true);
  };
